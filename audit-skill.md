...
Focus on actual code quality, source-level build quality, generated-binary quality, runtime behavior, maintainability, correctness, reliability, performance, security, source-level privacy leakage risks, compatibility, project-specific functional safety, and regression resistance.
...
- native/FFI boundary safety, parser safety, filesystem safety, dynamic-loading safety, service design, and privilege-boundary safety where applicable
...
- feature-level functional correctness, including whether implemented features work as intended, handle expected inputs and states, and fail safely
- user-facing GUI/UI behavior where applicable, including controls, forms, menus, navigation, state synchronization, user-triggered changes, and GUI-accessible actions with high blast radius
- project-specific functional risks, including cases where the program can cause instability, damage, data loss, security exposure, excessive cost, service disruption, unsafe hardware/software state, account lockout, or other harm even when features appear to work as designed
...
Identify previous diagnostic/debug/experimental/temporary features. Decide whether they should be removed, isolated, disabled, cleaned up, or left intact. They must not interfere with production behavior, correctness, performance, stability, security, privacy posture, binary quality, project-specific safety, or maintainability.
...
Do not dismiss crash, memory, lifetime, native-boundary, parser, concurrency, resource-exhaustion, binary-hardening, service-design, dynamic-loading, malformed-input, UI/action-flow, feature-functional, or project-specific functional-risk issues as theoretical if they can plausibly cause process crashes, data corruption, denial of service, privilege escalation, information disclosure, sandbox escape, account compromise, unsafe hardware/software behavior, unnecessary instability, or long-term maintenance risk.
...
[insert hot paths, auth, parsing, secrets, sensitive data, long-running jobs, persisted state, cloud/API cost paths, native/FFI code, parsers, untrusted file/network inputs, update mechanisms, plugins, dynamic loading, archive handling, deserialization, privileged operations, background services/daemons, drivers, hardware/financial/medical/industrial safety if applicable]

User-facing features and GUI/UI flows, if applicable:
[insert critical screens, controls, forms, menus, navigation paths, settings pages, destructive or high-blast-radius actions, user-triggered state changes, undo/redo or rollback expectations, frontend/backend synchronization requirements, accessibility-critical flows, and known UI/UX failure modes]

Domain-specific functional risks:
[insert project-specific ways the software can cause harm or instability even when it is “working as designed,” such as unsafe hardware settings, unstable overclock/undervolt profiles, unsafe fan curves, device misconfiguration, account lockouts, destructive automation, money movement, irreversible writes, unsafe privileged operations, excessive load generation, unsafe defaults, unsafe persistence across reboot, unsafe startup behavior, failed rollback, or user actions with high blast radius]

Hardware, driver, service, or low-level system-tool risks, if applicable:
[insert validation requirements for voltage/frequency/power/thermal/current limits, safe defaults, profile rollback, boot-time recovery, watchdog behavior, persistence safety, permission boundaries, driver/service stability, crash behavior, and prevention of unnecessary system instability]
...
2. Correctness, feature-level behavior, GUI/UI action flows where applicable, failure modes, timing, concurrency, backpressure
...
6. Architecture, maintainability, duplication, dead code, service design, and bloat
...
If context limits prevent a full audit, prioritize critical paths, blockers, crash/data-loss risks, security/privacy-leak/safety risks, memory corruption, use-after-free, buffer overflow/underflow, heap/stack corruption, double free, invalid free, stack overflow, integer bounds/allocation bugs, parser bugs, race/deadlock risks, unsafe service design, generated-binary hardening, source-build correctness, user-visible feature correctness, GUI/UI high-blast-radius action flows where applicable, and user-specified project-specific functional risks. State what was not inspected and lower confidence.
...
Review logic bugs, feature-level functional bugs, edge cases, bad assumptions, error handling, exceptions, state transitions, cleanup, crashes, regressions, undefined behavior, init/teardown, partial failures, malformed input, fragile integrations, cancellation/retry/restart/shutdown bugs, and bugs in logging/diagnostics/debug paths.

Also review uncaught exceptions, panics, aborts, fatal assertions, off-by-one errors, index/range/length/offset/count/capacity mistakes, integer overflow/underflow/truncation/sign-conversion bugs affecting allocation or bounds checks, incorrect state-machine transitions, unsafe fallback behavior, serialization/deserialization correctness, malformed/corrupted/oversized/truncated/deeply nested input, invalid config, debug/release behavior differences, platform/compiler/runtime differences, restart/migration/downgrade edge cases, and logic that accepts unsafe or nonsensical project-specific settings.

For user-facing software, also review feature-level functional correctness, GUI/UI event handling, rendering/state synchronization, navigation, form validation, enabled/disabled states, user-triggered state changes, undo/redo or rollback behavior where applicable, and cases where GUI-accessible actions fail, apply the wrong change, silently do nothing, desynchronize frontend/backend state, or trigger unsafe/high-blast-radius behavior.
...
Also review process crashes from unbounded recursion, stack exhaustion, heap/memory exhaustion, FD/socket/handle/thread/process/subprocess/timer/task exhaustion, repeated crash loops, unsafe signal handlers, cleanup/shutdown handlers that access destroyed state, callbacks/timers/tasks firing after owner destruction, cancellation leaving corrupted state or leaked resources, reentrancy bugs, lock-order inversion, starvation, priority inversion, UI/event-loop/request-thread blocking, TOCTOU reliability races, clock jumps, retry storms, cache stampedes, service restart loops, unsafe daemon/service lifecycle design, unsafe startup/shutdown behavior, and behavior when peers or dependencies hang, crash, return malformed data, or violate protocol expectations.

For user-facing software, also review whether GUI/UI-triggered operations remain reliable under slow operations, cancellation, repeated clicks, duplicate submissions, focus changes, navigation away/back, window close, sleep/wake, offline/online transitions, backend failure, partial saves, and concurrent user or background updates.
...
Also review excessive allocation churn, avoidable copying, unbounded result sets, missing pagination/streaming, N+1 queries, expensive synchronous work on UI/event-loop/request threads, lock contention, false sharing, cache-unfriendly hot paths, excessive logging/metrics/tracing/crash-report overhead, timer/wakeup storms, retries without backoff/jitter, cache stampedes, resource-amplification from malformed input, decompression bombs, regex backtracking blowups, algorithmic-complexity attacks, parser blowups, unbounded caches/logs/diagnostic buffers, binary size/bloat, unnecessary linked dependencies, unnecessary hardware load, unnecessary thermal/power/energy impact, and optimizations that make crash/security/memory behavior harder to reason about.

For user-facing software, also review avoidable UI jank, slow interactions, redundant re-rendering, layout thrashing, excessive polling, unbounded client-side state, blocking main-thread work, repeated expensive validation, excessive background refresh, and performance issues that make critical actions appear broken or encourage repeated submissions.
...
Also review unclear ownership/lifetime boundaries, unsafe or implicit resource ownership transfer, weak separation between trusted/untrusted data, weak separation between parsing/validation/authorization/execution, unsafe plugin/extension architecture, unsafe service/daemon/driver architecture, architecture that makes cancellation/shutdown/retry/recovery unsafe, unbounded queues/tasks/memory, callback/event designs prone to reentrancy or use-after-free, inconsistent async/error/cleanup conventions, leaky abstractions around storage/network/auth/filesystem/process/service/native boundaries, dead code reachable through reflection/plugins/generated code/dynamic loading/feature flags, duplicated parsers/validators/auth checks/crypto wrappers, unnecessary unsafe/native code, and insufficient isolation of high-risk parsers, native bindings, file extractors, crypto, auth, update logic, dynamic loading, services, drivers, privileged operations, GUI/action dispatch, state synchronization, or project-specific high-blast-radius features.
...
For GUI/UI projects, also review whether critical actions are reachable without a pointer device, focus order is safe and predictable, disabled/hidden state does not mask critical failures, status/error messages are perceivable, destructive or high-blast-radius actions are distinguishable and confirmable, and localization does not break layout or obscure safety/security-critical information.
...
Also review whether the program can cause unnecessary instability, damage, data loss, security exposure, excessive cost, service disruption, account lockout, device malfunction, degraded hardware lifetime, thermal/power/current/voltage/frequency limit violations, unsafe persistent settings, unsafe startup behavior, boot loops, failed recovery after crash/restart, or other project-specific harm even when operating according to its intended feature set.

For user-facing high-blast-radius software, also review whether GUI/UI-accessible actions can trigger unsafe settings, destructive operations, irreversible writes, unsafe persistence, privilege changes, unsafe automation, excessive cost, service disruption, or device/system instability; verify that validation, confirmation, rollback, emergency stop, recovery, and safe defaults are implemented where needed.

For hardware tuning, overclocking, undervolting, fan-control, firmware, driver, or low-level system tools, prioritize validation of voltage/frequency/power/thermal/current limits, safe defaults, profile rollback, boot-time recovery, watchdog behavior, persistence safety, permission boundaries, driver/service stability, crash behavior, and prevention of unnecessary system instability.
...
- Main feature-functional and GUI/UI blockers, if applicable
- Main project-specific functional-risk blockers
...
Explain confidence based on code inspected, builds/tests run, runtime behavior verified, binaries inspected, source-level analyzers run, sanitizer/fuzzer/static-analysis coverage checked, crash/security/memory-risk areas inspected, feature-level and GUI/UI behavior assessed where applicable, project-specific functional risks assessed, and categories not fully assessed.
...
- If user-facing GUI/UI behavior or critical feature workflows are central and were not meaningfully assessed, lower confidence and score the affected categories accordingly.
...
- If project-specific functional risks are central but not meaningfully assessed, lower confidence and score accordingly.
...
Verdict rules:
- Ready to ship: no Critical or High code/binary-quality blockers; no unmitigated serious crash/security/privacy-leak/memory-safety/feature-functional/GUI/project-specific safety blockers; source build/test/binary path sufficiently verified; remaining risks minor/acceptable.
...
- Not ready: unresolved High blocker, multiple Medium issues creating meaningful risk, insufficient confidence in testing/source-build/binary quality/security/reliability, insufficient confidence in feature-level or GUI/UI behavior for central flows, or insufficient confidence in memory/process-stability/project-specific risks for high-blast-radius code.
- Blocked: Critical blocker, severe data-loss/security/privacy-leak/safety risk, major memory corruption, high-confidence reachable crash, severe production instability, broken source build, broken generated binary, broken central feature workflow, unsafe GUI-accessible high-blast-radius behavior, unsafe project-specific behavior, missing essential source prerequisite, or unverified critical path.
...
Evidence must include concrete basis: file paths, functions, commands, source-build output, binary-inspection output, test output, observed behavior, static analysis output, sanitizer output, fuzzer output, dependency advisory output, relevant code patterns, GUI/UI behavior observed where applicable, feature-level behavior observed where applicable, or explicit absence of coverage. If unavailable, write "Evidence unavailable" and lower confidence.

Severity:
Critical = data loss, security compromise, privacy leak, unsafe physical/device/system state, major crash, severe instability, severe production failure, remotely reachable memory corruption, exploitable memory-safety issue, broken source build, broken generated binary, broken central feature behavior, unsafe GUI-accessible high-blast-radius behavior, unsafe project-specific behavior with high blast radius, or broken behavior.
High = serious correctness, reliability, security, privacy-leak, performance, source-build, binary-quality, safety, memory-safety, resource-exhaustion, feature-functional, GUI/UI, project-specific functional-risk, or maintainability issue.
...
- What feature-level functional or GUI/UI risks must be fixed before shipping, if applicable?
- What project-specific functional risks must be fixed before shipping?
...
- Which parts are fragile, risky, under-tested, costly, insecure, privacy-sensitive, platform-sensitive, memory-sensitive, crash-prone, concurrency-sensitive, parser-sensitive, native/FFI-sensitive, service-sensitive, binary-sensitive, GUI/UI-sensitive, feature-sensitive, domain-sensitive, or safety-sensitive?
...
Phases:
0. Safety and Baseline — establish tests, backups, reproducible source builds, generated-binary baselines, binary-inspection baselines, baseline behavior, known warnings/LSP errors/build output/generated artifacts/failures/performance-cost baselines, critical paths, high-risk modules, crash reproductions where available, sanitizer/static-analysis/fuzzer baselines where applicable. Identify memory-unsafe/native/FFI/parser/concurrency/resource-sensitive code, central feature workflows, GUI/UI high-blast-radius action flows where applicable, and project-specific functional-risk areas. Avoid behavior-changing refactors until validation exists.
1. Critical Fixes — fix blockers, crashes, broken source builds, broken binaries, data corruption, security/privacy-leak/safety risks, severe regressions, severe instability, memory corruption, use-after-free, buffer overflow/underflow, heap/stack corruption, double free, invalid free, exploitable parser bugs, unsafe deserialization, unsafe service behavior, broken central feature behavior, unsafe GUI/UI high-blast-radius behavior, unsafe project-specific behavior, and critical malformed-input/resource-exhaustion paths.
2. Correctness, Reliability, Compatibility — fix logic bugs, feature-functional bugs, GUI/UI action-flow bugs where applicable, state/lifecycle/error handling, failure recovery, timing, backpressure, compatibility, fragile behavior, cancellation/shutdown/retry/restart issues, malformed input handling, partial failure handling, service lifecycle behavior, binary compatibility, and crash-prone edge cases.
3. Regression Hardening — add/improve tests, static analysis, sanitizer coverage, linting, formatting, fuzzing, validation, malformed-input tests, resource-exhaustion tests, concurrency tests, parser tests, service lifecycle tests, feature-level behavior tests, GUI/UI regression tests where applicable, project-specific functional-risk tests, security regression tests, binary-inspection checks, and memory/lifetime regression tests. Do not include CI runner/infrastructure work unless asked.
4. Performance, Cost, Energy, Resources, and Binary Size — safe optimizations, lower overhead, improve memory/resource/storage I/O, reduce avoidable code-caused cloud/API/database/bandwidth/energy cost, remove bloat, reduce binary bloat where useful, and address algorithmic DoS, UI jank where applicable, unnecessary instability, or unbounded resource behavior.
5. Architecture and Maintainability — remove duplication, dead code, redundant code, diagnostic leftovers, fragile design, avoidable complexity, unclear ownership/lifetime boundaries, unsafe native/FFI abstraction leaks, unsafe service/driver boundaries, fragile GUI/UI state-management or action-dispatch boundaries where applicable, and risky parser/security/concurrency/domain abstractions. Refactor only when justified.
...
7. Accessibility, Internationalization, Source Docs, and Domain Safety Where Applicable — fix only applicable accessibility, localization, source-tree docs, config/troubleshooting docs, crash troubleshooting docs, corrupted-state recovery docs, sanitizer/fuzzer/binary-inspection docs, GUI/UI behavior docs where useful, domain-safety docs, and disaster recovery docs. Validate restore paths where implemented/documented in source.
8. Final Validation — run tests, clean source builds, binary inspection, static analysis, sanitizer/fuzzer validation where applicable, and verify performance/logs/storage/restore/compatibility/accessibility/i18n/feature-level behavior/GUI behavior/safety/domain behavior where applicable. Confirm no regressions, new crashes, privacy leaks, resource leaks, memory-safety regressions, binary-quality regressions, feature-functional regressions, GUI/UI regressions, project-specific safety regressions, or security regressions.
...
- Preserve behavior unless clearly wrong or unsafe.
...
- Add targeted regression tests for crashes, memory-safety bugs, parser bugs, malformed inputs, resource exhaustion, service lifecycle bugs, security bugs, binary-quality regressions, feature-functional bugs, GUI/UI action-flow bugs where applicable, project-specific safety risks, and concurrency bugs.
- Avoid worsening performance, runtime behavior, storage behavior, cost profile, accessibility/i18n where applicable, platform behavior, safety, source-build assumptions, binary quality, crash diagnosability, GUI/UI behavior where applicable, or sanitizer/static-analysis coverage.
- Preserve APIs, file formats, config formats, ABI expectations, binary compatibility expectations, user-visible feature behavior, GUI/UI behavior where applicable, and integration contracts unless breaking change is justified.
...
- Treat memory-unsafe/native/FFI/concurrency/parser/dynamic-loading/service/driver/domain-sensitive code as high-risk until inspected and tested.
- Treat GUI/UI actions that mutate state, invoke privileged operations, persist settings, or trigger high-blast-radius behavior as high-risk until inspected and tested.
- Prefer safe APIs over unsafe string/memory/path/process/dynamic-loading/service-control APIs.
- Prefer explicit bounds checks and checked arithmetic for allocation, indexing, parsing, serialization sizes, and domain-specific limits.
- Prefer bounded queues, bounded concurrency, backpressure, and safe rollback over unbounded growth or unsafe persistence.
...
- Do not introduce new unsafe code, native code, dynamic loading, shell execution, deserialization, service privilege, driver behavior, parser complexity, or high-blast-radius GUI/UI behavior unless justified and validated.
...
- Central feature workflows work as expected and fail safely.
- GUI/UI controls, forms, menus, navigation, enabled/disabled states, validation, rendering/state synchronization, user-triggered changes, undo/redo or rollback behavior where applicable, and high-blast-radius UI actions are tested or explicitly validated.
...
- Service, daemon, driver, privileged-helper, and background-task behavior is safe where applicable.
...
- Project-specific functional risks are validated, including unsafe defaults, unsafe persistence, failed rollback, crash/restart behavior, high-blast-radius settings, unnecessary instability, and recovery from bad settings where applicable.
- For hardware tuning, overclocking, undervolting, fan-control, firmware, driver, or low-level system tools, voltage/frequency/power/thermal/current limits, rollback, boot-time recovery, watchdog behavior, and prevention of unnecessary system instability are validated where applicable.
...
- Direct/transitive dependencies with parser, crypto, auth, native, filesystem, archive, deserialization, decompression, service, driver, or network-facing risk are checked for vulnerabilities and unsafe usage.
...
- Source-tree documentation is accurate enough for development, build, analysis, binary inspection, troubleshooting, crash diagnosis, memory-safety validation, sanitizer/fuzzer usage, service behavior, GUI/UI behavior where applicable, domain-safety validation, and recovery.
...
If a category cannot be fully assessed because code, source-build output, tests, runtime access, generated binaries, binary-inspection output, dependency data, production config, cost data, source-level privacy requirements, UI/localization requirements, platform requirements, hardware/domain requirements, native-symbol/debug data, sanitizer/fuzzer/static-analysis output, crash reproducers, feature-level behavior requirements, GUI/UI requirements, project-specific functional-risk requirements, or source-tree documentation are unavailable, say so and reduce confidence.
...
Do not omit relevant in-scope issues because they are pre-existing, non-critical, stylistic, outside main application code, hard to reproduce, platform-specific, sanitizer-only, static-analysis-only, binary-inspection-only, feature-functional, GUI/UI-specific, project-specific, or only reachable through malformed input.
